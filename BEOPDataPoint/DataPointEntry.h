...
public:
	...
	void SetEquipmentName(const wstring& strName);
	void SetSystemName(const wstring& strName);
	void SetGroupName(const wstring& strName);
	...
	wstring GetEquipmentName() const;

	wstring GetSystemName() const;

	wstring GetGroupName() const;
...
private:
	...
	wstring m_wstrOfEquipment;
	wstring m_wstrOfSystem;
	wstring m_wstrOfGroup;
...
enum OPCPOINTTYPE
{
	...
	POINTTYPE_WEB,
	POINTTYPE_WIRELESS,
	POINTTYPE_MODBUS_EQUIPMENT,
	POINTTYPE_THIRD_PARTY,
	POINTTYPE_UNKNOWN,
	POINTTYPE_PERSAGY_CONTROLLER,
	POINTTYPE_OPCUA,
	POINTTYPE_BACNET_PY
};

OPCPOINTTYPE GetPointType() const;
bool	IsUnknownPoint() const;
...
bool	IsWirelessPoint() const;
bool    IsModbusEquipmentPoint() const;
bool    IsOPCUAPoint() const;
bool	 IsBacnetPyPoint() const;
bool    IsPersagyControllerPoint() const;

bool    IsThirdPartyPoint() const;
...
public:
	...
	double	GetMultipleReadValue(double dOldValue, bool bRead=true) const;		//20190218 获取最终转换后的值
