...
bool CTCPDataSender::Init()
{
	...
	{
		_tprintf(L"ERROR: TCPDataSender connecting failed!\r\n");
		CDebugLog::GetInstance()->SetErrCode(ERROR_CUSTOM_CONNECT_TCPSENDER,true);
		return false;
	}
...
bool CTCPDataSender::ReConnect()
{
	...
	if (!TcpIpConnectComm(m_host, m_port))
	{
		_tprintf(L"ERROR: TCPDataSender reconnecting failed!\r\n");
		CDebugLog::GetInstance()->SetErrCode(ERROR_CUSTOM_RECONNECT_TCPSENDER,true);
		return false;
	}
	
	m_bConnectSuccess = true;
	//_tprintf(L"TCP Engine reconnecting success!\r\n");
	CDebugLog::GetInstance()->DebugLog(E_DEBUG_LOG_TCPSENDER,E_DEBUG_LOG_LEVEL_BASE,L"TCPDataSender reconnecting success!\r\n");
	return true;
}
...
bool CTCPDataSender::WriteToPort( char *pData,int nLen )
{
	...
	strDebugInfo.Format(_T("Run WriteToPort(Size:%d).\n"),nLen);
	...
	CDebugLog::GetInstance()->DebugLog(E_DEBUG_LOG_DTU,E_DEBUG_LOG_LEVEL_PROCESS,L"Exit WriteToPort.\n");
	return bResult;
}
...
bool CTCPDataSender::HandleRecData( const char* buffer,int nSize )
{
	if(buffer == NULL)
		return false;
	CDebugLog::GetInstance()->DebugLog(E_DEBUG_LOG_TCPSENDER,E_DEBUG_LOG_LEVEL_PROCESS,L"DEBUG:Run HandleRecData.\n");
	...
	CDebugLog::GetInstance()->DebugLog(E_DEBUG_LOG_TCPSENDER,E_DEBUG_LOG_LEVEL_PROCESS,L"DEBUG:Exit HandleRecData.\n");
	return true;
}
...
bool CTCPDataSender::UpdateDTUWriteTime()
{
...
void CTCPDataSender::SetHistoryDbCon( Beopdatalink::CCommonDBAccess* phisdbcon )
{
