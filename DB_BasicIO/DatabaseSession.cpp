...
CDatabaseSeesion::CDatabaseSeesion()
{
  ...
  unsigned int connectimeout = 20;
  unsigned int readwritetimeout = 20;
...
bool CDatabaseSeesion::Connect(Database_Param *pDbParam)
{
	...
	while(repeattime--)
	{
			...
			, pDbParam->client_flag))
		{
			...
			wstring wstrErr;
			Project::Tools::UTF8ToWideChar(strErr, wstrErr);
			_tprintf(wstrErr.data());
			// log the error.
...
{
	int nrc = mysql_query(&m_mysql, "start transaction;");
  return !nrc;
}

bool CDatabaseSeesion::SetIsolationCommited()
{
	//return !mysql_query(&m_mysql, "set global transaction isolation level read committed");
	int nrc = mysql_query(&m_mysql, "set global transaction isolation level read committed");
	return !nrc;
...
bool CDatabaseSeesion::AddMysqlErrLog( string strOperation, string strErrInfo )
{
	...
	if(Project::Tools::FindOrCreateFolder(strFilePath))
	{
		...
		GetLocalTime(&stNow);
		CString strLogPath;
		strLogPath.Format(_T("%s\\mysql_err_%d_%02d_%02d_Hour%02d.log"),strFilePath.c_str(),stNow.wYear,stNow.wMonth,stNow.wDay, stNow.wHour);
		CStdioFile	ff;
...
bool CDatabaseSeesion::AddMysqlErrSqlLog( string strOperation, string strErrInfo, string strErrSql )
{
	Project::Tools::Scoped_Lock<Mutex>  scopelock(m_lock);
	wstring strFilePath;
	Project::Tools::GetSysPath(strFilePath);
	...
	if(Project::Tools::FindOrCreateFolder(strFilePath))
	{
		...
		GetLocalTime(&stNow);
		CString strLogPath;
		strLogPath.Format(_T("%s\\mysqlsql_err_%d_%02d_%02d_Hour%02d.log"),strFilePath.c_str(),stNow.wYear,stNow.wMonth,stNow.wDay, stNow.wHour);
		CStdioFile	ff;
		if(ff.Open(strLogPath,CFile::modeCreate|CFile::modeNoTruncate|CFile::modeWrite))
		{
			wstring strTime,strOp,strEi,strSql;
			Project::Tools::SysTime_To_String(stNow,strTime);
			strOp = Project::Tools::AnsiToWideChar(strOperation.c_str());
			strEi = Project::Tools::AnsiToWideChar(strErrInfo.c_str());
			strSql = Project::Tools::AnsiToWideChar(strErrSql.c_str());
			CString strLog;
			strLog.Format(_T("%s::%s(%s)(%s)...\r"),strTime.c_str(),strOp.c_str(),strEi.c_str(),strSql.c_str());
			ff.Seek(0,CFile::end);
			ff.WriteString(strLog);
			ff.Close();
			return true;
