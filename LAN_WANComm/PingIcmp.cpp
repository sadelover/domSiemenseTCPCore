...
bool CPingIcmp::test_tcp_ip_port(string strIP, int nPort)
{
	...
	{  
		_tprintf(_T("test_tcp_ip_port: WSAStartup Failed\r\n"));
        return false;  
    }  
    ...
    {  
		...
		_tprintf(_T("test_tcp_ip_port: Socket Create Failed\r\n"));
        WSACleanup();//释放套接字资源  
        return  false;  
    }  
    ...
    servAddr.sin_addr.s_addr = inet_addr(strIP.data());  
	...
	{   
		_tprintf(_T("test_tcp_ip_port: socket connect Failed\r\n"));
...
bool CPingIcmp::PingV2(__in const string& strIpAddr)
{
	string strPingCommand = "ping "+ strIpAddr+" -w 2000 -n 1";
	if ( 0==system(strPingCommand.data()) ) {
	...
	ShExecInfo.lpParameters = L"/c ping 10.0.0.111 -w 5000 -n 1  >> D:\\disk.txt";
