	...
	if ((m_pServer->IsConnected ()) && (m_pIItemMgt != NULL))
	{
		...
		if (pItemArray != NULL)
		{
			...
			for (dwIndex = 0; dwIndex < dwCount; dwIndex++)
			{
				...
				pItemArray [dwIndex].vtRequestedDataType = VT_EMPTY; //类型不匹配错误
			}
				...
				else
				{
					...
					if (pErrors && FAILED (pErrors [dwIndex]))
					{
						...
						if(m_pServer)
						{
							...
							hr = m_pServer->GetIServer()->GetErrorString(pErrors[dwIndex], 0, &pErrorString);
							if(SUCCEEDED(hr))
							{
								CString strOut;
								strOut.Format(_T("ERROR: OPCServer Can't AddItem %s(0x%x:%s)\r\n"),pItem->GetItemID(),pErrors[dwIndex],pErrorString);			
								PrintLog(strOut);
							}
							else
							{
								CString strOut;
								strOut.Format(_T("ERROR: OPCServer Can't AddItem %s(0x%x)\r\n"),pItem->GetItemID(),pErrors[dwIndex]);
								PrintLog(strOut);
							}
						}
						else
...
bool CKGroup::ReadSync (CObArray &cItemList, DWORD cdwItems, bool bDeviceRead, bool bPostMsg /* = true */)
{
	...
	if(!m_pServer->IsConnected ())
		...
		for (dwIndex = 0; dwIndex < cdwItems; dwIndex++)
		{
			...
			// Set server handle:
			m_dwBuffer [dwIndex] = pItem->GetServerHandle ();
			if(m_dwBuffer[dwIndex] == NULL)
			{
				sqlstream << Project::Tools::WideCharToAnsi(pItem->GetItemID()) << "(" << dwIndex << ",0x0),";

				CString strOut;
				strOut.Format(_T("ERROR: OPCItem(%s) server handle is Null\r\n"),pItem->GetItemID());			
				PrintLog(strOut);
			}
			else
			{
				CString strOut;
				strOut.Format(_T("%s(%d,0x%x),"),pItem->GetItemID(),dwIndex,pItem->GetServerHandle());		
				sqlstream << Project::Tools::WideCharToAnsi(strOut);
			}
		}
		sqlstream << ")";
		CDebugLog::GetInstance()->SetDebugLog(Project::Tools::AnsiToWideChar(sqlstream.str().c_str()));
		...
		try
		{
			...
			if(SUCCEEDED (hr))
			{
				...
				else
				{
					...
					std::ostringstream sqlstream_reveive;
					sqlstream_reveive << "OPC Receive (";
					for (dwIndex = 0; dwIndex < cdwItems; dwIndex++)
					{
						// Get pointer to item in input list:
						CKItem *pItem = (CKItem *) cItemList [dwIndex];
						...
						if(pItem == NULL)
						{
							sqlstream_reveive  << "NULL(" << dwIndex << "),";
							continue;
						}
						...
						if(pErrors && SUCCEEDED(pErrors[dwIndex]))
						{
							...
							sqlstream_reveive << Project::Tools::WideCharToAnsi(pItem->GetItemID())  << "(" << dwIndex << "," << Project::Tools::WideCharToAnsi(strTmpVal) << "),"; 
						}
						else
						{
							// Request succeeded, but on or more items could not be read:
							if(pErrors[dwIndex] != NULL)
							{
								if(pErrors[dwIndex] == OPC_E_INVALIDHANDLE)
								{
									if(pItem)
									{
										m_mapItemError[pItem] = 1;
									}
								}
							}
							...
							if(pErrors && FAILED(pErrors[dwIndex]))
							{
								if(m_pServer)
								{
									...
									if(SUCCEEDED(hr))
									{
										bReadSuccess = bReadSuccess||false;//golding test: 当opc点没有配置在下位机中时，会到这里
										CString strOut;
										strOut.Format(_T("ERROR: OPCServer Can't read %s(0x%x:%s)\r\n"),pItem->GetItemID(),pErrors[dwIndex],pErrorString);			
										PrintLog(strOut);
									}
									else
									{
										bReadSuccess = bReadSuccess||false;//golding test: 当opc点没有配置在下位机中时，会到这里
										CString strOut;
										strOut.Format(_T("ERROR: OPCServer Can't read %s(0x%x)\r\n"),pItem->GetItemID(),pErrors[dwIndex]);
										PrintLog(strOut);
									}
								}
								else
								{
									...
									strOut.Format(_T("ERROR: OPCServer Can't read %s(0x%x)\r\n"),pItem->GetItemID(),pErrors[dwIndex]);
									PrintLog(strOut);
									CDebugLog::GetInstance()->SetErrPoint(pItem->GetItemID(),POINTTYPE_OPC_,ERROR_CUSTOM_UNEXIST_OPC_ITEM);
								}
							}
							else			// Request failed:
							{
						...
						//VariantClear (&pValues[dwIndex]);
						//dwValueIndex++;
					}
					sqlstream_reveive << ")";
					CDebugLog::GetInstance()->SetDebugLog(Project::Tools::AnsiToWideChar(sqlstream_reveive.str().c_str()));
				}
			}
			else
			{
				...
				{
					CString strOut;
					strOut = _T("ERROR: Called object(opc) has been disconnected from its clients.\r\n");
					PrintLog(strOut);
					CDebugLog::GetInstance()->SetErrCode(ERROR_CUSTOM_OPCSERVICE_DISCONNECT);
				}
				else if(hr == 0x800706be)		//远程过程调用失败。
				{
					CString strOut;
					strOut = _T("ERROR: Call the remote procedure(opc) fails.\r\n");
					PrintLog(strOut);
					CDebugLog::GetInstance()->SetErrCode(ERROR_CUSTOM_OPCSERVICE_FAIL);
				}
				else if(hr == 0x800706ba)		//RPC 服务器不可用
				{
					CString strOut;
					strOut = _T("ERROR: RPC server(opc) is unavailable\r\n");
					PrintLog(strOut);
					CDebugLog::GetInstance()->SetErrCode(ERROR_CUSTOM_PRC_FAIL);
				}
				else
				{
					CString strOut;
					strOut.Format(_T("ERROR: OPC Server Read Fail(0x%x)\r\n"),hr);
					PrintLog(strOut);
				}
			}
		}
		...
		if (bPostMsg && cdwSuccess !=cdwItems)
		{
			...
			//	cdwSuccess, GetName ());
			_tprintf(L"ERROR: KGroup ReadSync Read MutilValue num (%d/%d) Failed.\r\n",cdwSuccess,cdwItems);
			//bReadSuccess = false;
		}
				...
				else
				{
					...
					if (pErrors && FAILED (pErrors [cdwSentItems]))
					{
						if(m_pServer)
						{
							LPWSTR		pErrorString = NULL;
							//returns error texts for OPC and windows errors
							hr = m_pServer->GetIServer()->GetErrorString(pErrors[cdwSentItems], 0, &pErrorString);
							if(SUCCEEDED(hr))
							{
								CString strOut;
								strOut.Format(_T("ERROR: OPCServer Can't write %s(0x%x:%s)\r\n"),pItem->GetItemID(),pErrors[cdwSentItems],pErrorString);			
								PrintLog(strOut);
							}
							else
							{								
								CString strOut;
								strOut.Format(_T("ERROR: OPCServer Can't write %s(0x%x)\r\n"),pItem->GetItemID(),pErrors[cdwSentItems]);
								PrintLog(strOut);
							}
						}
						else
...
//  void
// **************************************************************************
bool CKGroup::ReadAsync10 ( CObArray &cItemList, DWORD cdwItems, bool bDeviceRead )
{
	ASSERT ( cdwItems > 0 );
	...
	if ( m_pServer->IsConnected () && m_pIAsync )
	{
		...
		try
		{
			...
			{
				// Check results:
				if(pErrors==NULL)//golding add, 如果opc服务中途中断，会返回在pErrors=NULL, pValues = NULL.
				{
					bReadSuccess = false;
				}
				else
...
//  void
// **************************************************************************
bool CKGroup::ReadAsync20 (CObArray &cItemList, DWORD cdwItems)
{
	ASSERT (cdwItems > 0);
	bool bReadSuccess = false;
	// There is no sence in building a request unless we are connected to
	...
	if (m_pServer->IsConnected () && m_pIAsync2)
	{
		...
		try
		{
			...
			else
			{
				...
				//	GetName (), hr);
				CString strOut;
				strOut.Format(_T("ERROR: OPCServer RefreshAsync20 Fail(0x%x)\r\n"),hr);
				PrintLog(strOut);
			}
		}
...
bool CKGroup::GetErrorItem( CObArray& cItemList )
{
	cItemList.RemoveAll();
	if(m_mapItemError.size() <= 0)
		return false;
	hash_map<CKItem*,int>::iterator iter = m_mapItemError.begin();
	while(iter != m_mapItemError.end())
	{
		cItemList.Add((*iter).first);
		iter++;
	}
	m_mapItemError.clear();
	return true;
...
IKDataSink20* CKGroup::GetIKDataSink20()
{
	return m_pIDataSink20;
}

bool CKGroup::GetUpdateTime20( COleDateTime& oleUpdate ,bool& bUpdateTimeAsync20, bool bOutAsync20)
{
	if(m_pIDataSink20)
		return m_pIDataSink20->GetUpdateTime20(oleUpdate,bUpdateTimeAsync20,bOutAsync20);
	return false;
}

bool CKGroup::GetAsync20ErrorItem( CObArray &cItemList )
{
	if(m_pIDataSink20)
		return m_pIDataSink20->GetAsync20ErrorItem(cItemList);
	return false;
}

bool CKGroup::GetSync20ErrorItem( CObArray &cItemList )
{
	if(m_pIDataSink20)
		return m_pIDataSink20->GetSync20ErrorItem(cItemList);
	return false;
}

void CKGroup::PrintLog( const CString &strLog_,bool bSaveLog /*= true*/ )
{
	_tprintf(strLog_);
	if(bSaveLog)
	{
		try
		{
			...
			if(Project::Tools::FindOrCreateFolder(strPath))
			{
				...
				CString strLogPath;
				strLogPath.Format(_T("%s\\opc_err_%d_%02d_%02d.log"),strPath.c_str(),oleNow.GetYear(),oleNow.GetMonth(),oleNow.GetDay());

				CString strLog;
				CString strTime;
				strTime.Format(_T("%02d:%02d:%02d"),oleNow.GetHour(),oleNow.GetMinute(),oleNow.GetSecond());
				strLog.Format(_T("%s::%s"),strTime,strLog_);
...
bool CKGroup::GetAsync20WriteItem( CObArray &cItemList )
{
	if(m_pIDataSink20)
		return m_pIDataSink20->GetAsync20WriteItem(cItemList);
	return false;
}
