...
namespace Project
{
	namespace Tools
	{
		...
		inline void GetModuleExeFileName(wstring &strFileName)
		{
			wchar_t szExePath[MAX_PATH] = {0};
			::GetModuleFileName(NULL, szExePath, MAX_PATH);

			wstring strSysPath = szExePath;
			int nFindIndex = strSysPath.find_last_of('\\');
			strFileName = strSysPath.substr(nFindIndex+1);
		}
		...
		inline wstring GetParentPath(wstring strSysPath)
		{
			
			int nFindIndex = strSysPath.find_last_of('\\');
			wstring wstrParentPath = strSysPath.substr(0,nFindIndex);
			return wstrParentPath;
		}
		...
		*/
		...
		inline wstring RemoveDecimalW(double value,int decprec=2)
		{
			...
			int nPresion = strValue.length();	//默认有效数据位
		...
		inline string WideCharToUtf8(const wchar_t* source)
		{
			try
			{
				...
				int nSourceLen = (int)wcslen(source);
				if(nSourceLen<=0)
					return "";
		...
		inline bool IsValidIP(const char* str)  
		{  		
			int i, a[4];
			char end;
		...
		inline void PrintLog(const wstring &strLog,bool bSaveLog = true)
		{
		...
		inline std::wstring ReplaceInString( const wstring& orignStr, const wstring& oldStr, const wstring& newStr, bool& bReplaced )
		{
			size_t pos = 0;
       ...
       inline  CString KGetComputerName()
		...
		inline BOOL StringInVector(wstring wstrItem, vector<wstring> wstrList)
		{

			for(int i=0;i<wstrList.size();i++)
			{
				if(wstrList[i]==wstrItem)
				{
					return true;
				}
			}
		...
		inline BOOL CalculateRealByScaleOrMapString(CString strScaleMapDef, double fDataTestValue,  double &fDataRealValue)
		{
			if(strScaleMapDef.GetLength()<=0)
				return FALSE;

			if(strScaleMapDef.Find(_T("|"))>0)
			{
				vector<wstring> wstrArray;
				SplitStringByChar(strScaleMapDef, '|', wstrArray);

				vector<double> fTestList;
				vector<double> fRealList;

				for(int i=0;i<wstrArray.size();i++)
				{
					CString strOneItem = wstrArray[i].c_str();
					vector<wstring> trMapArray;
					SplitStringByChar(strOneItem, ':', trMapArray);
					if(trMapArray.size()==2)
					{
						fTestList.push_back(_wtof(trMapArray[0].c_str()));
						fRealList.push_back(_wtof(trMapArray[1].c_str()));
					}

				}

				if(fTestList.size()>=2 && fRealList.size()>=2)
				{

					if(fabs(fTestList[1] - fTestList[0])>1e-5)
					{
						fDataRealValue = fRealList[0]+ (fDataTestValue - fTestList[0])*(fRealList[1]-fRealList[0])/(fTestList[1]-fTestList[0]);
						if(fDataRealValue< min(fRealList[0], fRealList[1]))
							fDataRealValue = min(fRealList[0], fRealList[1]);
						else if(fDataRealValue> max(fRealList[0], fRealList[1]))
							fDataRealValue = max(fRealList[0], fRealList[1]);
						return TRUE;
					}
				}
			}
			...
			{
				double fScale = _wtof(strScaleMapDef.GetString());
				if(fabs(fScale)>1e-5)
				{
					fDataRealValue = fDataTestValue/fScale;
					return TRUE;
				}
			}
		...
		inline BOOL CalculateTestByScaleOrMapString(CString strScaleMapDef, double fDataRealValue,  double &fDataTestValue)
		{
			if(strScaleMapDef.GetLength()<=0)
				return FALSE;
			...
			if(strScaleMapDef.Find(_T("|"))>0)
			{
				...
				if(fTestList.size()>=2 && fRealList.size()>=2)
				{
					...
					if(fabs(fRealList[1] - fRealList[0])>1e-5)
					{
						fDataTestValue = fTestList[0]+ (fDataRealValue - fRealList[0])*(fTestList[1]-fTestList[0])/(fRealList[1]-fRealList[0]);
						if(fDataTestValue>max(fTestList[0], fTestList[1]))
							fDataTestValue = max(fTestList[0], fTestList[1]);
						else if(fDataTestValue< min(fTestList[0], fTestList[1]))
							fDataTestValue = min(fTestList[0], fTestList[1]);
						return TRUE;
					}
				}
			}
			else
			{
				...
				if(fabs(fScale)>1e-5)
				{
					fDataTestValue = fScale* fDataRealValue;
					return TRUE;
				}
			}
